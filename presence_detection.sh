...
#thnx to Team Kleijn for testing the script
...
if [ "$show_debug" = 1 ]; then
	...
	echo ""
	echo "Overview of status Fritbox vs Domoticz"
	echo "----------------------------------------------------------------------------------" 
	printf '%s %s %s %s %s\n' "${devices_output[@]}" | column -t
	echo "----------------------------------------------------------------------------------" 
	echo "Note! Depending on how many devices are on the network it can take up to 10 minutes before you see a  OFF status change"
	echo "the ON status change is within seconds, this is a fritzbox issue"
...
if [ "$run_install" = 1 ]; then
	...
	# This is a user friendly installer for presence detection fritzbox 2.5
	...
	echo " Don't forget to disable wifi security (local wifi network) on your (mobile)devices"
	echo "-----------------------------------------------------------------------------------"
	#check if config file exist.
	...
	echo "Searching for FritzBox..."
	check_gateway=$(python $cwd/fritzconnection.py -i $get_gateway)
	#questions for config file 
	while [[ $check_gateway == *"None"* ]]; do
		echo -e "${RED}No FritzBox found!${NC}"
		echo -n "Enter ip adress  of Fritzbox and press [ENTER]:"  
		read ip_fritzbox
		check_gateway=$(python $cwd/fritzconnection.py -i $ip_fritzbox)
	...
	# check if in while above a password isset
	if [ -z $pass_fritzbox ]; then
		echo -e "Found FritzBox on:${GREEN}$get_gateway${NC}" 
		echo "ip_fritzbox=$get_gateway" >> config.txt
		echo "pass_fritzbox=test" >> config.txt
		ip_fritzbox=$get_gateway
		pass_fritzbox="test"
		else
		echo -e "Found FritzBox on:${GREEN}$ip_fritzbox${NC}" 
		echo "ip_fritzbox=$ip_fritzbox" >> config.txt
		echo "pass_fritzbox=$pass_fritzbox" >> config.txt
	fi
   
	read -e -p "Enter ip adres of domoticz including port and press [ENTER] default:" -i "127.0.0.1:8080" ip_domoticz
	echo "ip_domoticz=$ip_domoticz" >> config.txt
	## added by Allesvanzelf	
	echo -n "Enter the Domoticz Light/Switch protection code if you don't have any leave blank and press[ENTER]: "
    ...
    while [ "$number_of_devices" = 0 ] || [ -z "$number_of_devices" ];
	do
		echo -e "${RED}Number of devices must be 1 or higher!${NC}"
		echo -n "Enter number of devices you want to monitor and press [ENTER]:"
		read number_of_devices
	...
	do
		...
		echo -n "Enter mac address of device$i for example: 20:5E:A8:C1:AE:C0 and press [ENTER]:" 
		read -a mac
		device_mac+=("$mac")
	...
	echo "Run crontab -e and add:  * * * * * $cwd/presence_detection.sh  to your crontab. "
	echo "All the settings are written inside the config.txt. You can change or add settings there" 
